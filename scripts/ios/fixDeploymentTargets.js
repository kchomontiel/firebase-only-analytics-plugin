...
module.exports = function (context) {
  ...
  // Only run for iOS platform - FIXED: Check if platforms exists
  ...
  try {
    ...
    if (!fs.existsSync(iosProjectPath)) {
      console.log(
        "FirebasePlugin: iOS platform not found, skipping deployment target fix"
      );
      deferral.resolve();
      return deferral.promise;
    }
  ...
  if (matches) {
    matches.forEach(function (match) {
      ...
      if (oldValue !== newValue) {
        ...
        modified = true;
        console.log(
          "FirebasePlugin: Updated deployment target from",
          oldValue,
          "to",
          newValue
        );
